...
window.addEventListener('DOMContentLoaded', async () => {
  showPhoneDebug("Module script loaded successfully.", false);
  
  try {
    if (typeof MiniKit !== 'undefined') {
      MiniKit.install(WORLD_APP_ID);
      showPhoneDebug("MiniKit.install executed.", false);
    } else {
      showPhoneDebug("MiniKit is undefined from CDN!");
    }
  } catch (e) {
    showPhoneDebug("Install exception: " + e.message);
  }
  ...
  if (typeof MiniKit !== 'undefined' && MiniKit.isInstalled()) {
    ...
    showPhoneDebug("MiniKit.isInstalled() is FALSE! Open inside World App.");
...
function setUserData(username, address){
  ...
  showPhoneDebug("User connected: " + myAddress, false);
}
...
async function performWalletAuth(){
  ...
  if (!MiniKit.isInstalled()) {
    showPhoneDebug("Auth failed: MiniKit not installed.");
  ...
  try {
    ...
    showPhoneDebug("Auth response missing address.");
    return false;
  } catch (err) {
    showPhoneDebug("Auth error: " + err.message);
    return false;
  }
}
...
async function handlePlayButtonClick() {
  ...
  alert("Ready to play! Address: " + myAddress);
}
